  ...
  ({ columns, data }: ITableProps): ReactElement => {
    ...
    const handleOpenVariant = useCallback(
      ({ index }: PropsRow) => {
        ...
        const idx = isFiltered()
          ? toJS(mainTableStore.filteredNo)[index]
          : index
    ...
    useEffect(() => {
      ...
      const handleResize = debounce(() => {
        mainTableStore.setIsTableRecizing(true)

        setTimeout(() => {
          mainTableStore.setIsTableRecizing(false)
        }, 500)
      }, 500)
    ...
    const RenderRow = useCallback(
      ({ index, style }) => {
        ...
        const isLoading =
          mainTableStore.tabReport.lastPage?.isFetching &&
          index === rows.length - 1
                    ...
                    {isSampleColumn ? (
                      <div onClick={stopPropagation}>
                        <ScrollContainer
                          style={{
                            cursor: `${
                              valueNumber > 3 ? 'grabbing' : 'pointer'
                            }`,
                            overflow: `${
                              valueNumber > 3 ? 'hidden' : 'visible'
                            }`,
                          }}
                        >
                          {cell.render('Cell')}
    ...
    const handleScrollAsync = debounce(async () => {
      if (mainTableStore.tabReport.hasNextPage) {
        mainTableStore.tabReport.addPage()
        return
      }
    }, 100)
    ...
    return (
      ...
      >
        ...
        {toJS(mainTableStore.tabReport.pages).length > 0 && (
          <Autosizer>
            {({ height }) => (
